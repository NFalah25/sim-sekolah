...
@section('content')
    <div class="bg-white border-b mt-4 border-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <p class="text-sm text-gray-500">
                ...
                <span class="font-semibold text-gray-800">Struktur Organisasi</span>
            </p>
        </div>
    </div>
    <main class="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="text-center max-w-3xl mx-auto mb-12">
            ...
            <p class="text-slate-500 text-lg">Mengenal lebih dekat susunan kepemimpinan, manajemen, dan tata usaha yang
                bersinergi memajukan sekolah.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-9">
                <div id="content-container"
                    class="bg-white rounded-2xl shadow-sm border border-gray-100 p-1.5 md:p-3 h-full transition-all duration-500">
                    ...
                    <div class="px-4 py-4 md:px-6 border-b border-gray-50 flex justify-between items-center">
                        ...
                        <div
                            class="hidden md:flex items-center gap-2 text-xs text-slate-400 bg-slate-50 px-3 py-1.5 rounded-full">
                            <i data-lucide="zoom-in" class="w-4 h-4"></i> Klik gambar untuk memperbesar
                        </div>
                    </div>

                    <!-- Image Container -->
                    <div class="chart-container relative w-full bg-slate-50 rounded-xl overflow-hidden mt-2 group cursor-zoom-in"
                        onclick="openZoomModal()">
                        <img id="display-image" src="" alt="Bagan Organisasi"
                            class="w-full h-auto object-contain min-h-[400px] md:min-h-[600px] transition-transform duration-500 group-hover:scale-[1.02]">

                        <div
                            class="zoom-overlay absolute inset-0 bg-black/20 opacity-0 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
                            <button
                                class="bg-white text-primary px-6 py-3 rounded-full font-bold shadow-lg flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
    ...
    <!-- MODAL ZOOM (Lightbox) -->
    <div id="zoom-modal"
        class="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm hidden p-4">
        ...
        <div class="w-full h-full overflow-auto flex items-center justify-center" onclick="closeZoomModal()">
            <img id="modal-image" src="" alt="Full Size Chart"
                class="max-w-none md:max-w-full md:max-h-full shadow-2xl rounded-lg cursor-default"
                onclick="event.stopPropagation()">
        </div>
        <div
            class="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-black/50 text-white px-4 py-2 rounded-full text-sm backdrop-blur-md pointer-events-none">
...
@push('scripts')
    <script>
        ...
        const structures = @json($jsonStruktur);
        ...
        function renderSidebar() {
            ...
            structures.forEach((item, index) => {
                ...
                // Set default class
                btn.className =
                    `tab-btn w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-left font-semibold text-slate-600 bg-white border border-transparent transition-all duration-300 ${index === 0 ? 'active' : ''}`;
        ...
        function updateContent(data) {
            ...
            displayImage.src = data.images; // Jika dari laravel pakai: asset('storage/' + data.image)
        }
